...
## Exemplo 2: Estimando um Integral

Queremos estimar
$$
\theta=\int_{0}^{\infty} e^{-x^2}dx,
$$
cujo valor exato é $\theta=\sqrt{\pi}/2$.
Para isso, usaremos $X\sim\mathrm{Exp}(1)$, que tem densidade $f_X(x)=e^{-x}$ para $x\ge 0$.
Note que
$$
\theta=\int_{0}^{\infty} e^{-x^2}dx
...
$$
\widehat{\theta}=\frac{1}{n}\sum_{i=1}^n e^{-(X_i^2-X_i)},\qquad X_i\stackrel{iid}{\sim}\mathrm{Exp}(1).
$$
...
set.seed(58)
N <- 1:2000
theta_hat_seq <- numeric(length(N))
x <- numeric(length(N))

for (i in seq_along(N)) {
  x[i] <- rexp(1, rate = 1)
  theta_hat_seq[i] <- mean(exp(-(x[1:i]^2 - x[1:i])))
}

plot(N, theta_hat_seq, type = "l", xlab = "n", ylab = expression(hat(theta)[n]),
     main = expression(Aproximacao~de~theta))
abline(h = sqrt(pi)/2, col = "red", lty = 2)
```
...
np.random.seed(58)
N = np.arange(1, 2001)
theta_hat_seq = np.zeros_like(N, dtype=float)
x = np.array([])

for i in range(len(N)):
    x = np.append(x, np.random.exponential(scale=1.0))
    theta_hat_seq[i] = np.mean(np.exp(-(x**2 - x)))

plt.plot(N, theta_hat_seq)
plt.axhline(y=np.sqrt(np.pi)/2, linestyle='--')
plt.xlabel('n')
plt.ylabel(r'$\hat{\theta}_n$')
plt.title(r'Aproximação de $\theta$')
plt.show()
...
## Exemplo 3: Aproximando uma Probabilidade 
...
## Exemplo 4: Aproximando o valor de $\pi$
...
- Utilize o método de Monte Carlo para aproximá-la

- Forneça um intervalo de confiança para avaliar a precisão da estimativa.
Compare com o valor real da integral.

- Crie 10000 estimativas de Monte Carlo para $I$, cada uma com base em 100 observações, junto com seus intervalos de confiança. Qual a proporção delas que caem dentro do IC? 

- Repita o item anterior com 1000 ao inveś de 100.
...
Seja $Z\sim N(0,1)$. Mostre que sua FGM é $E[e^{tZ}]=e^{t^2/2}$. Para $t\in\{-1,0.5,1,1.5\}$, estime $E[e^{tZ}]$ por Monte Carlo, forneça ICs e compare com a fórmula fechada. Estude o decaimento do erro quando $n$ cresce.



<span style="color:#2C92B2;">**Exercício 6**</span>. 

Seja $X_1,\ldots,X_n$ uma amostra iid a $X\sim \text{Gamma}(2,1)$. Calcule

- uma cota superior para $P(\bar X_n-2>0.3)$ usando Markov

- uma cota superior para $P(\bar X_n-2>0.3)$ usando Chebyshev

- uma aproximação para $P(\bar X_n-2>0.3)$ usando o teorema do limite central

- uma aproximação para $P(\bar X_n-2>0.3)$ usando Monte Carlo, com um número de simulações $B=10000$.

Comente quem é mais informativo em cada $n$.


<span style="color:#2C92B2;">**Exercício 7**</span>. 

(Desafio)
Sejam $X_1,\ldots,X_n\stackrel{iid}{\sim}\text{Exp}(\lambda)$. 

- Mostre que a esperança de $X_{(k)}$ (a k-ésima estatística de ordem) é $\sum_{j=1}^k \frac{1}{\lambda(n-j+1)}$.
Dica: Pense nos espaçamentos $Y_i=X_{(i)}-X_{(i-1)}$ com $X_{(0)}=0$.
Mostre que $\min(X_1,\dots,X_n)\sim \text{Exp}(n\lambda)$ e, depois,
use a falta de memória: depois que o mínimo “sai”, restam (n-1) expoenciais iid $\text{Exp}(\lambda)$. Conclua que
   $   Y_i\sim \text{Exp}((n-i+1)\lambda)\quad\Rightarrow\quad E[Y_i]=\frac{1}{(n-i+1)\lambda}.$. Finalmente, note que $X_{(k)}=\sum_{i=1}^k Y_i$ e some as esperanças.

- Estime $E[X_{(5)}]$ para $(n,k,\lambda)=(50,5,2)$ via Monte Carlo e compare com o valor teórico. Forneça IC.
