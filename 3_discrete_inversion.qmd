...
Este método funciona pois $x_i$ é o  menor valor $x_i$ tal que $F(x_i) \geq u$
se, e só se, $F(x_i) \geq u \geq F(x_{i=1})$.
Assim, a probabilidade do valor gerado por esse algoritmo ser $x_i$ 
é

$$P(F(x_i) \geq U \geq F(x_{i=1}))=F(x_i) - F(x_{i-1})= \sum_{j=1}^{i} p(x_j) - \sum_{j=1}^{i-1} p(x_j)=p(x_i).$$
...
Como $k$ precisa ser um número inteiro (já que a distribuição geométrica conta o número de tentativas), usamos a função de arredondamento "para cima" ($\lfloor \cdot \rfloor$), conhecida como a **função piso**:
