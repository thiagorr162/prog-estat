...
**Enunciado.** Considere uma v.a. discreta \$X\$ com suporte ordenado \$x\_1\<x\_2<\cdots\$ e massas \$p\_i=\mathbb P(X=x\_i)\$. Defina a CDF por \$F(x\_i)=\sum\_{j\le i}p\_j\$ e \$F(x)=0\$ para \$x\<x\_1\$. Seja \$U\sim\mathrm{Unif}(0,1)\$ e defina o algoritmo
...
$$
\mathbb P(\tilde X=x_i)
=\mathbb P\big(F(x_{i-1})<U\le F(x_i)\big)
=F(x_i)-F(x_{i-1})
=p_i.
$$

Portanto \$\tilde X\$ reproduz exatamente a PMF desejada. ▢
