...
Para tanto, a ideia principal é gerar pares de amostras $X_i$ e $Y_i$ que sejam correlacionadas negativamente. Concretamente, se tivermos uma segunda amostra $Y_1, ..., Y_n$ i.i.d entre sim
e com $E[Y] = E[X] = \theta$ e $V[Y] = V[X] = \sigma^2$, mas não necessariamente independente da sequência  $X_i$,
...
$$
onde $\rho = \text{Corr}(X, Y)$. Assim, se $\rho < 0$, então $\text{Var}(\hat{\theta}_A) < \text{Var}(\hat{\theta})$, tornando este estimador **mais eficiente** que o estimador de Monte Carlo usual com uma amostra de tamanho $2n$.
