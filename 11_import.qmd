...
$$
em que $Y$ tem densidade $g$. Isso vale desde que $g$ possua suporte tão ou mais amplo que $f(x)$.
Assim, podemos estimar $\E[h(X)]$ usando $n$ valores simulados de $Y$ independentemente pela fórmula:
...
Gráfico de $h(x) f(x)$ e as funções de importância $g(x)$ e $g2(x)$:
Aqui está o código convertido para Python usando a biblioteca `matplotlib` e `seaborn` no formato solicitado:
...
Aqui está o código equivalente em Python usando `matplotlib` no formato solicitado:
